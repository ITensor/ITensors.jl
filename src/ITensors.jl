"""
ITensor is a library for rapidly creating correct and efficient tensor network
algorithms.
...
and transforming tensor networks, such as matrix product state and matrix
product operators.
...
# Other Features of ITensor
  ...
  - Tools for **tensor networks**, such as matrix product states (MPS)
    / tensor trains (TT)
  ...
  - ITensors can have **sparse data** internally, such as block sparsity or
    diagonal sparsity, while having the same interface as dense ITensors
  - ITensors can have **symmetry properties** (invariance or equivariance) under
    group transformations of the indices. In physics terminology such ITensors
