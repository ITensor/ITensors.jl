...
length(D::Dense) = length(D.data)
data(D::Dense) = D.data

copy(D::Dense{T}) where {T} = Dense{T}(copy(D.data))

storage_convert(::Type{Array},D::Dense,inds::IndexSet) = reshape(data(D),dims(inds))

function storage_getindex(Tstore::Dense,Tinds::IndexSet,vals::Int...)
  return getindex(reshape(data(Tstore),dims(Tinds)),vals...)
end

function storage_setindex!(Tstore::Dense,Tinds::IndexSet,x::Number,vals::Int...)
  return setindex!(reshape(data(Tstore),dims(Tinds)),x,vals...)
end
...
function storage_add!(Bstore::Dense,Binds::IndexSet,Astore::Dense,Ainds::IndexSet)
  p = calculate_permutation(Binds,Ainds)
  Adata = data(Astore)
  Bdata = data(Bstore)
  if is_trivial_permutation(p)
    Bdata .+= Adata
  else
    reshapeBdata = reshape(Bdata,dims(Binds))
    permAdata = permutedims(reshape(Adata,dims(Ainds)),p)
...
function storage_permute!(Bstore::Dense,Binds::IndexSet,Astore::Dense,Ainds::IndexSet)
  p = calculate_permutation(Binds,Ainds)
  Adata = data(Astore)
  Bdata = data(Bstore)
  if is_trivial_permutation(p)
    ...
    reshapeBdata = reshape(Bdata,dims(Binds))
    permutedims!(reshapeBdata,reshape(Adata,dims(Ainds)),p)
  end
end

function storage_dag(Astore::Dense,Ainds::IndexSet)
  return dag(Ainds),storage_conj(Astore)
end
...
function storage_contract(Astore::TensorStorage,Ainds::IndexSet,Bstore::TensorStorage,Binds::IndexSet)
  (Alabels,Blabels) = compute_contraction_labels(Ainds,Binds)
  (Cinds,Clabels) = contract_inds(Ainds,Alabels,Binds,Blabels)
  Cstore = contract(Cinds,Clabels,Astore,Ainds,Alabels,Bstore,Binds,Blabels)
  return (Cinds,Cstore)
end
