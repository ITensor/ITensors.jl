function qn_svdMPO(os::OpSum{C}, sites; kwargs...)::MPO where {C}
  ...
  for n in 1:N
    ...
    for term in sequence(os)
      crosses_bond(term, n) || continue

      left::Prod{Op} = filter(t -> (site(t) < n), sequence(term))
      onsite::Prod{Op} = filter(t -> (site(t) == n), sequence(term))
      right::Prod{Op} = filter(t -> (site(t) > n), sequence(term))
  ...
  for n in 1:N
    ...
    for el in tempMPO[n]
      ...
      M = get!(finalMPO, (el.rowqn, sequence(t)), defaultMat(ll, rl, el.rowqn, el.colqn))
