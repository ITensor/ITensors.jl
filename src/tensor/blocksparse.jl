...
struct BlockSparse{T} <: TensorStorage
  data::Vector{T}
  offsets::Vector{Tuple{Int,Int}}  # Block number-offset pairs
  BlockSparse{T}(data,offsets) where {T} = new{T}(convert(Vector{T},data),offsets)
  BlockSparse{T}() where {T} = new{T}(Vector{T}(),Vector{Tuple{Int,Int}}())
end

# Convenient functions for BlockSparse storage type
Base.@propagate_inbounds Base.getindex(D::BlockSparse,i::Integer) = data(D)[i]
Base.@propagate_inbounds Base.setindex!(D::BlockSparse,v,i::Integer) = (data(D)[i] = v)

Base.similar(D::BlockSparse{T}) where {T} = BlockSparse{T}(similar(data(D)))
...
Base.copyto!(D1::BlockSparse,D2::BlockSparse) = copyto!(data(D1),data(D2))

Base.fill!(D::BlockSparse,v) = fill!(data(D),v)
...
function Base.:*(D::BlockSparse{<:El},x::S) where {El<:Number,S<:Number}
  return BlockSparse{promote_type(El,S)}(x*data(D))
end
...
function BlockSparseTensor(locs::Vector{NTuple{N,Int}},
                           inds::NTuple{N,NTuple{<:Any,Int}}) where {N}
  offsets = Vector{Tuple{Int,Int}}()
  block_strides = Base.size_to_strides(1,length.(inds)...)
  stride_offset = 1-sum(block_strides)

  linear_offset_total = 1
  for jj in 1:length(locs)
    linear_loc = sum(locs[jj] .* block_strides)+stride_offset
    push!(offsets,(linear_loc,linear_offset_total))
    current_block_length = 1
    for dim = 1:N
      current_block_length *= inds[dim][locs[jj][dim]]
    end
...
Base.fill!(T::BlockSparseTensor,v) = fill!(store(T),v)
