...
#function slice_block(block::Block{N},
#                     slicedims::NTuple{NS,Int}) where {N,NS}
...
function combine_dims(blocks::Blocks{N},
  ...
  for (i,block) in enumerate(blocks)
    ...
    block_comb = insertafter(block_comb,tuple(slice_comb),combdims[1]-1)
