...
function contract(
        As::Union{Vector{ITensor}, Tuple{Vararg{ITensor}}}; sequence = default_sequence(),
        kwargs...
    )::ITensor
...
function contract!(C::ITensor, A::ITensor, B::ITensor, α::Number, β::Number = 0)::ITensor
    ...
    CT = NDTensors.contract!!(
        tensor(C), _Tuple(labelsC), tensor(A), _Tuple(labelsA), tensor(B),
        _Tuple(labelsB), α, β
    )
...
function product(A::ITensor, B::ITensor; apply_dag::Bool = false)
    ...
    isempty(commonindsAB) &&
        error("In product, must have common indices with prime level 0.")
