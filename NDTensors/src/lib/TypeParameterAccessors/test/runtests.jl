...
if VERSION ≥ v"1.8"
  ...
  @testset "Test NDTensors.TypeParameterAccessors" begin
    ...
    @testset "Set parameter at position" begin
      @test ((() -> set_parameters(Array{Float32,3}, (Float16, 2)))()) == Array{Float16,2}
      @test ((() -> set_parameters(Array, (Float32, 3)))()) == Array{Float32,3}
      @test (set_parameters(Array{Float32,2})) == Array{Float32,2}

      @test ((() -> set_parameter(Array{Float32,3}, 1, Float16))()) == Array{Float16,3}
      @test ((() -> set_parameter(Array{Float32,3}, 2, 2))()) == Array{Float32,2}
      @test ((() -> set_parameter(Array{Float32}, 1, Float16))()) == Array{Float16}
      @test ((() -> set_parameter(Array{Float32}, 2, 2))()) == Array{Float32,2}
      @test ((() -> set_parameter(Vector, 1, Float16))()) == Array{Float16,1}
      @test ((() -> set_parameter(Vector, 2, 2))()) == Matrix
      @test ((() -> set_parameter(Array, 1, Float16))()) == Array{Float16}
      @test ((() -> set_parameter(Array, 2, 2))()) == Matrix

      @test ((() -> set_parameter(Vector, Position(1), Float32))()) == Vector{Float32}
      @test ((() -> set_parameter(Array{Float32}, Position(2), TypeParameter(2)))()) ==
        Array{Float32,2}
      @test ((() -> set_parameter(Array{Float32}, Float16))()) == Array{Float16}
      @test ((() -> set_parameter(Array{<:Any,2}, Position(2), TypeParameter(3)))()) ==
        Array{<:Any,3}
    end

    @testset "Set ndim and eltype" begin
      @test ((() -> set_ndims(Array{Float32,2}, 3))()) == Array{Float32,3}
      @test ((() -> set_eltype(Array{Float32,2}, Float16))()) == Array{Float16,2}
      @test ((() -> set_ndims(Array, 2))()) == Matrix
      @test (((() -> set_eltype(Array, Float32))())) == Array{Float32}
      m = Transpose(Matrix{Float32}(undef, (2, 3)))
      @test (() -> set_eltype(typeof(m), Float16))() == Transpose{Float16,Matrix{Float16}}
      @test ((() -> set_eltype(typeof(m), Float16))()) == Transpose{Float16,Matrix{Float16}}
      @test ((() -> set_parameter(typeof(m), eltype, Float16))()) ==
        Transpose{Float16,Matrix{Float32}}
      ## TODO This code does not infer the correct type but there aren't any allocations
      ## When it is called so I believe its actually working properly
      ## In a wrapped function on the command line this does however show the correct
    ...
    @testset "Set a parameter if it is unspecified" begin
      @test (specify_parameter(Array{Float32,3}, 1, Float16)) == Array{Float32,3}
      @test (specify_parameter(Array{Float32,3}, 2, 2)) == Array{Float32,3}
      @test ((() -> specify_parameter(Array{Float32}, 1, Float16))()) == Array{Float32}
      @test ((() -> specify_parameter(Array{Float32}, 2, 2))()) == Array{Float32,2}
      @test ((() -> specify_parameter(Array, 1, Float16))()) == Array{Float16}
      @test ((() -> specify_parameter(Array, 2, 2))()) == Array{<:Any,2}
      @test ((() -> specify_parameter(Matrix, 1, Float32))()) == Array{Float32,2}
      @test ((() -> specify_parameter(Matrix, 2, 2))()) == Array{<:Any,2}
      @test ((() -> specify_parameter(Vector, 1, Float64))()) == Array{Float64,1}
      @test ((() -> specify_parameter(Vector, 2, 2))()) == Array{<:Any,1}

      @test ((() -> specify_parameter(Array, eltype, Float32))()) == Array{Float32}
      @test ((() -> specify_parameter(Array, ndims, 2))()) == Array{<:Any,2}

      @test specify_parameter(Array{<:Any,3}, 1, Float16) == Array{Float16,3}
      @test specify_parameter(Array{<:Any,3}, 2, 2) == Array{<:Any,3}
      ## TODO Do we want to support type stable this behavior, it does not seem suppported by julia.
      ## Though if you have `Matrix` or `Vector` the code is type stable
      @test (specify_parameter(Array{<:Any,3}, Position(1), Float16)) == Array{Float16,3}
      @test (specify_parameter(Array{<:Any,3}, Position(2), 2)) == Array{<:Any,3}
    end

    @testset "Set multiple parameters if they are unspecified" begin
      @test ((() -> specify_parameter(specify_parameter(Array, 1, Float32), 2, 2))()) ==
        Array{Float32,2}
      @test ((() -> specify_parameter(specify_parameter(Array, 2, 2), 1, Float32))()) ==
        Array{Float32,2}

      @test ((() -> specify_parameter(specify_parameter(Matrix, 2, 3), 1, Float32))()) ==
        Array{Float32,2}
      @test ((() -> specify_parameter(specify_parameter(Matrix, 1, Float32), 2, 3))()) ==
        Array{Float32,2}
      @test ((() -> specify_parameter(specify_parameter(Vector, 1, Float32), 2, 3))()) ==
        Array{Float32,1}
      @test ((
        () -> specify_parameter(specify_parameter(Array, eltype, Float32), ndims, 2)
      )()) == Matrix{Float32}
      ...
      @test ((() -> specify_default_parameters(Array))()) == Vector{Float64}
      @test ((() -> specify_default_parameters(Matrix))()) == Matrix{Float64}
      @test ((() -> specify_default_parameters(Array{Float32}))()) == Vector{Float32}
