...
function inferred(f::Function) 
  ...
  @testset "Test NDTensors.TypeParameterAccessors" begin
    ...
    @testset "Set ndim and eltype" begin
      ...
      @test_broken (() -> set_eltype(typeof(m), Float16))() == Transpose{Float16,Matrix{Float16}}
    ...
    @testset "Set a parameter if it is unspecified" begin
      @test @inferred(specify_parameter(Array{Float32,3}, Position(1), Float16)) == Array{Float32,3}
    ...
    @testset "Set multiple parameters if they are unspecified" begin
      ...
      @test inferred((() -> specify_parameters(Array, (1,2), (Float32,2)))) == Array{Float32,2}
      @test inferred((() -> specify_parameters(Array, (2,1), (2,Float32)))) == Array{Float32,2}

      @test inferred((() -> specify_parameters(Matrix, (1,2), (Float32,3)))) == Array{Float32,2}
      @test inferred((() -> specify_parameters(Matrix, (2,1), (3,Float32)))) == Array{Float32,2}
    
      @test inferred(
        (() -> specify_parameters(Array, (eltype, ndims), (Float32,2)))) == Matrix{Float32}
      @test inferred(
        (() -> specify_parameters(Array, (ndims,eltype), (2,Float32)))) == Matrix{Float32}

      @test inferred(
        (() -> specify_parameters(Array, (Position(1), ndims), (Float32,2)))) == Matrix{Float32}
      @test inferred(
        (() -> specify_parameters(Array, (2,Position(1)), (2,Float32)))) == Matrix{Float32}
        @test inferred(
        (() -> specify_parameters(Array, (1, ndims), (Float32,2)))) == Matrix{Float32}
      ...
      @test @inferred((() -> specify_default_parameters(Array{Float32}))()) ==
        Vector{Float32}
    ...
    @testset "Testing MyTypeNamedPositions" begin
      ...
      @test @inferred(set_parameter(m, eltype, Float32)) == MyTypeNamedPositions{'T',Float32, 'D'}
      @test set_parameter(m, third, 'S') == MyTypeNamedPositions{'T', 'N', 'S'}
      @test specify_parameters(m, (ndims, third), (2, 's')) == m
      @test specify_parameters(MyTypeNamedPositions, (ndims, third), (2, 's')) == MyTypeNamedPositions{2, <:Any, 's'}
    end
    ...
    @testset "Testing MyTypeDefaultPositions" begin
      ...
      @test @inferred(set_parameter(m, Position(2), Float32)) == MyTypeDefaultPositions{'T',Float32, 'D'}
      @test set_parameter(m, 3, 'S') == MyTypeDefaultPositions{'T', 'N', 'S'}
      @test specify_default_parameters(MyTypeDefaultPositions) == MyTypeDefaultPositions{2, Float16, 'S'}
      @test specify_default_parameters(MyTypeDefaultPositions, (Position(2), Position(1))) == MyTypeDefaultPositions{2, Float16}
      @test specify_default_parameters(MyTypeDefaultPositions{<:Any, <:Any, 'L'}, (Position(2), Position(1))) == MyTypeDefaultPositions{2, Float16, 'L'}
    ...
    @testset "Testing MyTypeDefaultNamedPosition" begin
      ...
      @test @inferred(set_parameter(m, eltype, Float32)) == MyTypeDefaultNamedPosition{1,Float32,3}
      @test set_parameter(m, third, 'S') == MyTypeDefaultNamedPosition{1, 2, 'S'}
      @test set_parameters(m, (third, ndims, eltype), ('S', 5, Float32)) == MyTypeDefaultNamedPosition{5, Float32, 'S'}
      @test specify_parameter(m, third, Float32) == m
      @test specify_parameter(MyTypeDefaultNamedPosition, eltype, Float32) == MyTypeDefaultNamedPosition{<:Any, Float32}

      @test specify_default_parameter(MyTypeDefaultNamedPosition, eltype) == MyTypeDefaultNamedPosition{<:Any, Float32}
      @test specify_default_parameter(MyTypeDefaultNamedPosition, third) == MyTypeDefaultNamedPosition{<:Any, <:Any, 'P'}
      @test specify_default_parameters(MyTypeDefaultNamedPosition, (ndims, third)) == MyTypeDefaultNamedPosition{3, <:Any, 'P'}
      
      TypeParameterAccessors.position_name(::Type{<:MyTypeDefaultNamedPosition}, ::Position{1}) = ndims
      TypeParameterAccessors.position_name(::Type{<:MyTypeDefaultNamedPosition}, ::Position{2}) = eltype
      TypeParameterAccessors.position_name(::Type{<:MyTypeDefaultNamedPosition}, ::Position{3}) = third
      @test specify_default_parameters(MyTypeDefaultNamedPosition) == MyTypeDefaultNamedPosition{3, Float32, 'P'}
