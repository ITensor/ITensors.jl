using .TypeParameterAccessors: set_ndims, specify_default_type_parameters, unwrap_array_type
...
function generic_randn(arraytype::Type{<:AbstractArray}, dims...; rng=Random.default_rng())
  arraytype_specified = set_ndims(
    specify_default_type_parameters(unwrap_array_type(arraytype)), length(dims)
  )
