...
@testset "Empty QN ITensor storage operations" begin
  ...
  storage(C)
  storage(C)
  @test storage(C) isa BlockSparse{
    NDTensors.UnspecifiedZero,
    NDTensors.UnallocatedZeros{
      NDTensors.UnspecifiedZero,1,Tuple{Base.OneTo{Int64}},Vector{NDTensors.UnspecifiedZero}
    },
    2,
  }
  ...
  @test hassameinds(C, (i'', i))
  @test storage(C) isa BlockSparse{
    Float64,NDTensors.UnallocatedZeros{Float64,1,Tuple{Base.OneTo{Int64}},Vector{Float64}},2
  }
...
@testset "blockoffsets" begin
  i = Index(2)
  A = ITensor(i', dag(i))
  @test_broken blockoffsets(A) == NDTensors.BlockOffsets{2}()
  i = Index([QN(0) => 1, QN(1) => 1])
  A = ITensor(i', dag(i))
