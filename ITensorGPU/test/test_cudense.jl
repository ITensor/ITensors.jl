...
@testset "cuITensor, Dense{$SType} storage" for SType in (Float64, ComplexF64)
  ...
  @testset "Test add CuDense" begin
    ...
    dA = ITensorGPU.CuDense{SType,CuVector{SType, ITensorGPU.default_buffertype()}}(SType(1.0), dim(i) * dim(j))
    B = [SType(2.0) for ii in 1:dim(i), jj in 1:dim(j)]
    dB = ITensorGPU.CuDense{SType,CuVector{SType,ITensorGPU.default_buffertype()}}(SType(2.0), dim(i) * dim(j))
  ...
  @testset "Test permute CuDense" begin
    ...
    dA = ITensorGPU.CuDense{SType,CuVector{SType, ITensorGPU.default_buffertype()}}(NDTensors.Dense(vec(A)))
    B = [SType(0.0) for ii in 1:dim(j), jj in 1:dim(j)]
    dB = ITensorGPU.CuDense{SType,CuVector{SType, ITensorGPU.default_buffertype()}}(SType(0.0), dim(i) * dim(j))
  ...
  @testset "Test basic CuDense features" begin
    @test NDTensors.Dense{SType,CuVector{SType, ITensorGPU.default_buffertype()}}(10) isa ITensorGPU.CuDense{SType}
